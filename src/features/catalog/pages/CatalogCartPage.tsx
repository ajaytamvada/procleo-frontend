...
const CatalogCartPage: React.FC = () => {
  ...
  const handleCreatePR = async () => {
    ...
    if (vendorIds.length > 1) {
      toast.error(
        'Catalog PR requires all items from the same vendor. Please remove items from different vendors.'
      );
      return;
    }
    ...
    try {
      ...
      const { data } = await apiClient.post(
        '/purchase/requests?sendForApproval=false',
        prPayload
      );
      clearCart();
      toast.success(
        `Purchase Request ${data.requestNumber || ''} created successfully!`
      );
              ...
              <div className='bg-white border border-gray-200 rounded-xl p-5 sticky top-20'>
                ...
                {[...new Set(cartItems.map(i => i.vendorId))].length > 1 && (
                  <div className='mt-3 p-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700'>
                    ⚠️ Items from multiple vendors. Catalog PRs require single
                    vendor.
                  </div>
                )}
