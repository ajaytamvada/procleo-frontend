...
import { getPermissionName, parsePermissions } from '../../constants/permissions';

interface UserPermissionListProps {
  onEditPermissions: (userType: UserType) => void;
}

const UserPermissionList: React.FC<UserPermissionListProps> = ({ onEditPermissions }) => {
  ...
  if (error) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12">
        <div className="text-center text-red-500">Failed to load user types</div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredAndPaginatedData.content.length === 0 ? (
              ...
              filteredAndPaginatedData.content.map((userType, index) => {
                const permissions = parsePermissions(userType.permissionClass ?? null);
                  ...
                  <tr key={userType.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium text-gray-900">{userType.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {permissions.length === 0 ? (
                        <span className="text-sm text-gray-500 italic">No modules assigned</span>
                      ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                        ...
                        {userType.status === '1' ? 'Active' : 'Inactive'}
                      </span>
                    </td>
      ...
      <div className="bg-gray-50 px-6 py-3 flex items-center justify-between border-t border-gray-200">
        <div className="text-sm text-gray-700">
          Showing {filteredAndPaginatedData.content.length} of {filteredAndPaginatedData.totalElements} results
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={page >= filteredAndPaginatedData.totalPages - 1 || filteredAndPaginatedData.totalPages === 0}
            className="px-4 py-2 border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 transition-colors"
          >
            Next
          </button>
