...
const currencyAPI = {
  ...
  create: async (currency: Omit<Currency, 'id'>) => {
    const response = await apiClient.post<Currency>('/master/currencies', currency);
    return response.data;
  },

  update: async (id: number, currency: Partial<Currency>) => {
    const response = await apiClient.put<Currency>(`/master/currencies/${id}`, currency);
    return response.data;
  },
  ...
  exportToExcel: async (filters: MasterEntityFilters = {}): Promise<Blob> => {
    ...
    const response = await apiClient.get(`/master/currencies/export?${params}`, {
      responseType: 'blob',
    });
    return response.data;
  },
...
export const useCurrencies = (page = 0, size = 15, filters: MasterEntityFilters = {}) => {
  return useQuery({
