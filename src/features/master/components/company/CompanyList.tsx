...
import { Plus, Search, Edit, Trash2, Power, Upload } from 'lucide-react';
import {
  useCompanies,
  useDeleteCompany,
...
const CompanyList: React.FC<CompanyListProps> = ({
  ...
  const handleToggleStatus = async (company: Company) => {
    ...
    if (
      window.confirm(`Are you sure you want to ${action} "${company.name}"?`)
    ) {
        ...
        companiesData.totalPages > 1 && (
          <div className='px-6 py-3 flex items-center justify-between border-t border-gray-200'>
            <div className='flex items-center text-sm text-gray-500'>
              Showing{' '}
              {(companiesData.size || 0) * (companiesData.number || 0) + 1} to{' '}
              {Math.min(
                (companiesData.size || 0) * ((companiesData.number || 0) + 1),
                companiesData.totalElements || 0
              )}{' '}
              of {companiesData.totalElements || 0} results
            </div>
            <div className='flex space-x-2'>
              <button
              ...
              <button
                onClick={() =>
                  setPage(
                    Math.min((companiesData.totalPages || 1) - 1, page + 1)
                  )
                }
