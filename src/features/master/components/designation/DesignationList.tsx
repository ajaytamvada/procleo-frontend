...
interface DesignationListProps {
  ...
  onFiltersChange?: (filters: DesignationFilters) => void;
}
...
const DesignationList: React.FC<DesignationListProps> = ({
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await designationAPI.exportToExcel(filters);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `designations_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      alert('Failed to export designations. Please try again.');
    } finally {
  ...
  return (
    <div className='min-h-screen bg-[#f8f9fc] p-2'>
      ...
      <div className='flex items-center justify-between mb-6'>
        <div>
          <h1 className='text-xl font-semibold text-gray-900'>Designation</h1>
      ...
      <div className='bg-white rounded-lg border border-gray-200 overflow-hidden'>
        <div className='overflow-x-auto'>
          {isLoading ? (
            ...
            <div className='flex flex-col items-center justify-center py-16'>
              ...
              <p className='text-gray-600 font-medium'>No designations found</p>
              <p className='text-gray-400 text-sm mt-1'>
                Click "New Designation" to create one
              </p>
            </div>
          ) : (
        ...
        {/* Pagination */}
        {totalPages > 0 && !isLoading && !error && designations.length > 0 && (
