...
const paymentTermSchema = z.object({
  name: z.string().min(1, 'Payment term is required').max(255, 'Cannot exceed 255 characters'),
});
...
const PaymentTermPage: React.FC = () => {
  ...
  const [selectedPaymentTerm, setSelectedPaymentTerm] = useState<PaymentTerm | undefined>(undefined);
  const [page, setPage] = useState(0);
  const [nameFilter, setNameFilter] = useState('');

  // Fetch payment terms
  const { data, isLoading, error, refetch } = usePaymentTerms(page, 20, nameFilter);
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900">
              {selectedPaymentTerm ? 'Edit Payment Term' : 'Create Payment Term'}
            </h2>
          </div>
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Manage payment terms for purchase orders and invoices</p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        <p className="text-sm text-blue-800">
          Showing <span className="font-semibold">{paymentTerms.length}</span> of{' '}
          <span className="font-semibold">{totalElements}</span> payment terms
        </p>
      </div>
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-sm">Create your first payment term to get started</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={paymentTerm.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        ...
                        <button
                          onClick={() => paymentTerm.id && handleDelete(paymentTerm.id)}
