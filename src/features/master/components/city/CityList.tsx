...
import type { City, CityFilters } from '../../hooks/useCityAPI';
import { cityAPI } from '../../hooks/useCityAPI';
...
const CityList: React.FC<CityListProps> = ({
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await cityAPI.exportToExcel(filters);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `cities_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      alert('Failed to export cities. Please try again.');
    } finally {
  ...
  return (
    <div className='min-h-screen bg-[#f8f9fc] p-2'>
      ...
      <div className='bg-white rounded-lg border border-gray-200 overflow-hidden'>
        <div className='overflow-x-auto'>
          {isLoading ? (
            <div className='flex flex-col items-center justify-center py-16'>
              ...
              <p className='text-sm text-gray-500 mt-3'>Loading cities...</p>
            </div>
          ) : cities.length === 0 ? (
            <div className='flex flex-col items-center justify-center py-16'>
        ...
        {totalPages > 0 && !isLoading && cities.length > 0 && (
                ...
                <React.Fragment key={idx}>
                  {pageNum === '...' ? (
                    ...
                    <button
                      ...
                      className={`min-w-[36px] h-9 px-3 rounded-lg text-sm font-medium transition-colors ${page === pageNum ? 'bg-violet-600 text-white border border-violet-600' : 'text-gray-600 border border-gray-200 hover:bg-gray-50 hover:border-gray-300'}`}
                    >
                      {(pageNum as number) + 1}
                    </button>
                  )}
