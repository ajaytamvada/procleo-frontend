...
const termsAndConditionsSchema = z.object({
  content: z.string().min(1, 'Terms and conditions text is required').max(1000, 'Cannot exceed 1000 characters'),
  fileName: z.string().max(255, 'File name cannot exceed 255 characters').optional().or(z.literal('')),
});
...
const TermsAndConditionsPage: React.FC = () => {
  ...
  const [selectedTC, setSelectedTC] = useState<TermsAndConditions | undefined>(undefined);
  const [page, setPage] = useState(0);
  const [contentFilter, setContentFilter] = useState('');

  // Fetch terms and conditions
  const { data, isLoading, error, refetch } = useTermsAndConditions(page, 20, contentFilter);
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900">
              {selectedTC ? 'Edit Terms & Conditions' : 'Create Terms & Conditions'}
            </h2>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                You can reference a document file name if you've uploaded T&C documents separately
              </p>
            </div>
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Manage terms and conditions for purchase orders and contracts</p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        <p className="text-sm text-blue-800">
          Showing <span className="font-semibold">{termsAndConditions.length}</span> of{' '}
          <span className="font-semibold">{totalElements}</span> terms and conditions
        </p>
      </div>
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center">
                      <p className="text-lg mb-2">No terms and conditions found</p>
                      <p className="text-sm">Create your first terms and conditions to get started</p>
                    </div>
                  </td>
                </tr>
              ) : (
