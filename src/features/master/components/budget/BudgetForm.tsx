...
const budgetSchema = z.object({
  ...
  annualBudget: z.number()
    .positive('Annual budget must be greater than zero'),
});
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 mt-1">
          {mode === 'create' ? 'Add a new budget to the system' : 'Update budget information'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
              ...
              render={({ field }) => (
                <select
                  ...
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                    errors.financialYearId ? 'border-red-300' : 'border-gray-300'
                  }`}
        ...
        {mode === 'edit' && budget && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4">
            <p className="text-sm text-yellow-800">
              <strong>Note:</strong> Financial Year and Department cannot be changed when editing.
              Only the Annual Budget amount can be updated.
            </p>
          </div>
        )}
        ...
        <div className="border-t pt-6 flex justify-end space-x-4">
          ...
          >
            {isLoading ? 'Saving...' : mode === 'create' ? 'Create Budget' : 'Update Budget'}
          </button>
        </div>
      </form>
    </div>
