...
import { Edit2, Trash2, Search, Download, ChevronLeft, ChevronRight, Users, X } from 'lucide-react';
import type { AssignmentGroup, AssignmentGroupFilters } from '../../hooks/useAssignmentGroupAPI';
import {
...
const AssignmentGroupList: React.FC<AssignmentGroupListProps> = ({ onEdit }) => {
  ...
  const { data, isLoading, error } = useAssignmentGroupsPaged(page, pageSize, filters);
  const deleteMutation = useDeleteAssignmentGroup();

  const { data: members = [], isLoading: loadingMembers } = useGroupMembers(viewMembersGroupId!);
          ...
          >
            ...
            <div className="p-6">
              {loadingMembers ? (
                ...
                <div className="text-center text-gray-500">No members in this group</div>
              ) : (
                <div className="space-y-2">
                  {members.map((member) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600">Code: {member.employeeCode}</div>
                      <div className="text-sm text-gray-600">Email: {member.employeeEmail}</div>
                      {member.contactNumber && (
                        <div className="text-sm text-gray-600">Phone: {member.contactNumber}</div>
                      )}
                    </div>
                  ))}
                </div>
