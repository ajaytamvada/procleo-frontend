...
const CreatePurchaseOrderPage: React.FC = () => {
  ...
  const handleRFPChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedRfpId) {
      ...
      // Clear RFP related data if needed, or just do nothing
    }
  };

  const handleSupplierChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedSupplierId = parseInt(e.target.value);
    const selectedSupplier = vendors.find(v => v.id === selectedSupplierId);
  ...
  const handleItemChange = (
    ...
    const { name, value } = e.target;
    const numValue = ['quantity', 'unitPrice', 'tax1Rate', 'tax2Rate'].includes(
      name
    )
      ? parseFloat(value) || 0
      : value;
  ...
  const handleSubmit = async (shouldSubmitForApproval: boolean = false) => {
    try {
      ...
      if (formData.rfpNumber) {
        // Find the RFP ID from the approvedRFPs list based on the number
        const rfp = approvedRFPs.find(r => r.rfpNumber === formData.rfpNumber);
        if (rfp) {
          ...
          // Fallback to URL param if available
          createdPO = await createPOFromRFP.mutateAsync({
            rfpId: parseInt(rfpIdParam),
            data: poData,
          });
        } else {
          // Fallback to regular creation if RFP ID not found
    ...
    <div className='min-h-screen bg-gray-50'>
      ...
      <div className='max-w-7xl mx-auto px-6 py-6'>
              ...
              <div className='flex-1'>
                ...
                <div className='text-sm text-blue-700 space-y-1'>
                  ...
                  <p className='text-xs mt-2'>
                    ...
                    selected quotation. You can review and modify them before
                    creating the PO.
                  </p>
                </div>
              </div>
        ...
        <div className='bg-white rounded-lg shadow-sm border border-gray-200 mb-6'>
          ...
          <div className='p-6'>
            {/* Add Item Form - Only show if not from RFP */}
            {!formData.rfpNumber && (
