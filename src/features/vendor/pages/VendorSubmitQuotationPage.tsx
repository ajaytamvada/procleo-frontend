...
export default function VendorSubmitQuotationPage() {
  ...
  const rfpIdNum = rfpId ? parseInt(rfpId, 10) : undefined;

  const {
    data: rfp,
    isLoading: rfpLoading,
    error: rfpError,
  } = useVendorRFP(rfpIdNum);
  ...
  // Initialize items from RFP data
  useEffect(() => {
    if (rfp?.items && rfp.items.length > 0) {
      const initialItems: QuotationItemForm[] = rfp.items.map(
        (item: VendorRFPItem) => ({
          rfpItemId: item.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const quotationData = {
      quotationNumber: quotationNumber || `QTN-${Date.now()}`,
    ...
    return (
      <div className='p-6'>
        <div className='bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3'>
          ...
          <div>
            <h3 className='font-medium text-red-800'>Error Loading RFP</h3>
            <p className='text-red-600 text-sm mt-1'>
              {(rfpError as Error)?.message || 'Unable to load RFP details'}
            </p>
            <Link
              to='/vendor/rfp-invitations'
              className='text-red-700 hover:text-red-800 text-sm font-medium mt-2 inline-block'
            >
              ← Back to RFP Invitations
            </Link>
          </div>
        </div>
      </div>
  ...
  if (rfp.hasSubmittedQuotation) {
    return (
      <div className='p-6'>
        <div className='bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3'>
          <AlertCircle className='w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5' />
          <div>
            <h3 className='font-medium text-amber-800'>Already Submitted</h3>
            <p className='text-amber-600 text-sm mt-1'>
              You have already submitted a quotation for this RFP.
            </p>
            <Link
              to='/vendor/quotations'
              className='text-amber-700 hover:text-amber-800 text-sm font-medium mt-2 inline-block'
            >
              View My Quotations →
            </Link>
          </div>
    ...
    <div className='p-6 space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-4'>
          <Link
            to={`/vendor/rfp-invitations/${rfpId}`}
      ...
      <div className='bg-violet-50 border border-violet-200 rounded-lg p-4'>
        <div className='grid grid-cols-3 gap-4'>
          <div className='flex items-center gap-2'>
            ...
            <span className='text-sm text-gray-600'>RFP Number:</span>
            <span className='text-sm font-medium'>{rfp.rfpNumber}</span>
          </div>
          <div className='flex items-center gap-2'>
        ...
        <div className='bg-white border rounded-lg overflow-hidden'>
          <div className='p-4 border-b bg-gray-50'>
            ...
            <p className='text-sm text-gray-500'>
              Enter your prices for each item
            </p>
          </div>
        ...
        <div className='bg-white border rounded-lg p-6'>
          ...
          <div className='mt-6 flex justify-end gap-4'>
            ...
            <button
              type='submit'
              disabled={
                submitMutation.isPending || items.some(i => i.unitPrice <= 0)
              }
