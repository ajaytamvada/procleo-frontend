...
export const useNegotiateQuotation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(`Quotation ${quotation?.quotationNumber} marked for negotiation. Vendor will be notified.`);
    ...
    onError: (error: any) => {
      const errorMessage = error?.response?.data?.message || 'Failed to mark quotation for negotiation';
...
export const useEvaluateQuotations = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Quotations evaluated successfully for RFP ${data.rfpNumber}`);
    ...
    onError: (error: any) => {
      const errorMessage = error?.response?.data?.message || 'Failed to evaluate quotations';
      toast.error(errorMessage);
    },
  });
};
