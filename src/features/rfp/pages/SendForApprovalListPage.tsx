/**
 * Send For Approval List Page
 * Shows list of RFPs with submitted quotations ready for finalization
 */
...
import { useRFPsForFinalization } from '../hooks/useSendForApproval';

export const SendForApprovalListPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');

  const { data: rfps = [], isLoading, error } = useRFPsForFinalization();
  ...
  const handleFinalize = (rfpId: number) => {
    navigate(`/rfp/${rfpId}/finalize`);
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Finalize vendor selection and send RFPs for management approval
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-6">
          ...
          {filteredRFPs.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-1">
                {searchTerm ? 'No matching RFPs found' : 'No RFPs available for finalization'}
              </h3>
        ...
        {filteredRFPs.length > 0 && (
          <div className="mt-4 text-sm text-gray-600">
            Showing {filteredRFPs.length} RFP{filteredRFPs.length !== 1 ? 's' : ''} ready for
            finalization
          </div>
        )}
      </div>
...
export default SendForApprovalListPage;
