    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start">
          ...
          <div>
            <h3 className="text-red-800 font-medium">Error Loading RFP Summary</h3>
            <p className="text-red-600 text-sm mt-1">
              {error instanceof Error ? error.message : 'Unable to load RFP summary'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto p-6 print:p-0">
        <div className="bg-white shadow-sm rounded-lg p-8 print:shadow-none">
          ...
          <div className="border-b-2 border-gray-300 pb-4 mb-6">
            <h1 className="text-3xl font-bold text-gray-900 text-center">RFP SUMMARY</h1>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4 px-4">
              ...
              <div>
                ...
                <p className="font-semibold">{new Date(summary.rfpDate).toLocaleDateString()}</p>
              </div>
              <div>
                ...
                <div key={qIndex} className="mb-6 px-4">
                  <div className="bg-blue-50 p-3 rounded mb-2">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="font-semibold">Vendor:</span> {quotation.supplierName}
                      </div>
                      <div>
                        <span className="font-semibold">Vendor Code:</span> {quotation.supplierCode}
                      </div>
                      <div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 mb-2">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-2 py-2 text-right text-xs font-medium text-gray-500">Tax</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {quotation.items.map((item, iIndex) => (
                        <tr key={iIndex}>
                          ...
                          <td className="px-2 py-2 text-sm text-right">₹{item.unitPrice?.toFixed(2)}</td>
                          <td className="px-2 py-2 text-sm text-right">₹{item.taxValue?.toFixed(2)}</td>
                  ...
                  <div className="bg-gray-50 p-2 rounded">
                    <div className="flex justify-end space-x-8 text-sm">
                      <div>
                        ...
                        <span className="font-semibold">₹{quotation.otherCharges?.toFixed(2) || '0.00'}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Discount:</span>{' '}
                        <span className="font-semibold">₹{quotation.discount?.toFixed(2) || '0.00'}</span>
                      </div>
                      <div>
                        <span className="text-gray-600 font-bold">Grand Total:</span>{' '}
                        <span className="font-bold text-lg">₹{quotation.grandTotal?.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Total Bid Amount</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {summary.bidEvaluations.map((evaluation, index) => (
                      <tr key={index} className={evaluation.lowestBidRank === 1 ? 'bg-green-50' : ''}>
                        <td className="px-4 py-2 text-sm font-semibold">{evaluation.supplierName}</td>
                        <td className="px-4 py-2 text-sm text-right">₹{evaluation.totalBidAmount?.toFixed(2)}</td>
            ...
            <div className="mb-8">
              ...
              <div className="overflow-x-auto px-4">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Vendor</th>
                      <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Item</th>
                      <th className="px-2 py-2 text-right text-xs font-medium text-gray-500">Old Price</th>
                      <th className="px-2 py-2 text-right text-xs font-medium text-gray-500">New Price</th>
                      <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Notes</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {summary.negotiations.map((nego, index) => (
                      <tr key={index}>
                        <td className="px-2 py-2 text-sm">{nego.supplierName}</td>
                        <td className="px-2 py-2 text-sm">{nego.itemName}</td>
                        <td className="px-2 py-2 text-sm text-right">₹{nego.oldTotalPrice?.toFixed(2)}</td>
                        <td className="px-2 py-2 text-sm text-right font-semibold text-green-600">
                          ₹{nego.newTotalPrice?.toFixed(2)}
                        </td>
                        <td className="px-2 py-2 text-sm">{nego.negotiationNotes || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {summary.selectedVendors.map((vendor, index) => (
                      <tr key={index} className="bg-green-50">
                        <td className="px-4 py-2 text-sm font-semibold">{vendor.supplierName}</td>
                        <td className="px-4 py-2 text-sm">{vendor.itemName}</td>
                        <td className="px-4 py-2 text-sm">{vendor.selectionRemarks || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
