...
export const ReSubmitQuotationListPage: React.FC = () => {
  ...
  // Flatten to show one row per quotation
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        ...
        <span className="ml-2 text-gray-600">Loading submitted quotations...</span>
      </div>
    );
  }
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Modify and re-submit existing quotations</p>
          </div>
        </div>
      </div>
          ...
          {quotationRows.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-1">
                {searchTerm ? 'No matching quotations found' : 'No submitted quotations available'}
              </h3>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {quotationRows.map((row, index) => (
                    <tr key={`${row.quotation.id}-${index}`} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <Building2 size={16} className="text-gray-400 mr-2" />
                          <span className="text-sm font-medium text-gray-900">
                            {row.supplier?.supplierName || row.quotation.supplierName || `Supplier ${row.quotation.supplierId}`}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-600">
                            {row.quotation.quotationDate ? new Date(row.quotation.quotationDate).toLocaleDateString() : 'N/A'}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        ₹{(row.quotation.netAmount || row.quotation.totalAmount || 0).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          onClick={() => handleEditQuotation(row.rfp.id!, row.quotation.id!)}
