...
import {
  useQuotationForEdit,
  useUpdateQuotation,
} from '../hooks/useReSubmitQuotation';
...
export const ReSubmitQuotationFormPage: React.FC = () => {
  const { rfpId, quotationId } = useParams<{
    rfpId: string;
    quotationId: string;
  }>();
  ...
  const {
    ...
    error,
  } = useQuotationForEdit(rfpIdNum, quotationIdNum);
    ...
    if (data?.quotation && data?.rfp) {
      ...
      if (quotation.items && quotation.items.length > 0) {
        const initialItems: QuotationItemForm[] = quotation.items.map(item => ({
          ...
          unitOfMeasurement: data.rfp.items?.find(
            rfpItem => rfpItem.id === item.rfpItemId
          )?.unitOfMeasurement,
          unitPrice: item.unitPrice,
          originalUnitPrice: item.unitPrice, // Store original for validation
          targetUnitPrice:
            data.rfp.items?.find(rfpItem => rfpItem.id === item.rfpItemId)
              ?.unitPrice || 0,
    ...
    setItems(prevItems => {
      ...
      if (field === 'unitPrice') {
        ...
        if (newPrice > item.originalUnitPrice) {
          alert(
            `Unit price cannot be greater than original unit price (₹${item.originalUnitPrice.toFixed(2)})`
          );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const priceExceeded = items.find(
      item => item.unitPrice > item.originalUnitPrice
    );
    if (priceExceeded) {
      alert(
        `Unit price for "${priceExceeded.itemName}" cannot exceed original price`
      );
      return;
    }
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-600'>
                {data.quotation.supplierName || 'Supplier'}
              </p>
            </div>
          </div>
          <button
          ...
          <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start'>
            ...
            <div>
              ...
              <p className='text-yellow-700 text-sm mt-1'>
                New unit prices cannot be greater than the original unit prices.
                You can only reduce or maintain prices during re-submission.
              </p>
            </div>
          </div>
