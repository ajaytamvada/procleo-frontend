...
export const SendForApprovalPage: React.FC = () => {
  ...
  const [vendorSelections, setVendorSelections] = useState<{
    [itemId: number]: { supplierId: number; supplierName: string; remarks: string };
  }>({});
  ...
  const handleVendorSelection = (itemId: number, supplierId: number, supplierName: string) => {
    setVendorSelections({
      ...vendorSelections,
      [itemId]: {
        supplierId,
  ...
  const handleSubmit = () => {
    ...
    const selections = Object.entries(vendorSelections).map(([itemId, selection]) => ({
  ...
  const uniqueItemNames = Array.from(new Set(allItems.map((item) => item.itemName)));
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Send For Approval</h1>
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">RFP Information</h2>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-gray-500 uppercase">Quotations Received</p>
                <p className="mt-1 text-sm font-semibold text-green-600">{quotations.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900">Submitted Quotations</h2>
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-purple-600">
                          ₹
                          {(quotation.netAmount || quotation.totalAmount || 0).toLocaleString(
                            'en-IN',
                            {
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Finalize Vendor Selection</h2>
              ...
              {uniqueItemNames.map((itemName) => {
                ...
                return (
                  <div key={itemName} className="border border-gray-200 rounded-lg p-4">
                    <h3 className="font-medium text-gray-900 mb-3">{itemName}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          value={vendorSelections[firstItem.id!]?.supplierId || ''}
                          onChange={(e) => {
                            const supplierId = parseInt(e.target.value);
                            const supplier = quotations.find((q) => q.supplierId === supplierId);
                            if (supplier) {
                        ...
                        >
                          ...
                          {itemQuotations.map(({ quotation, item }) => (
                            <option key={quotation.supplierId} value={quotation.supplierId}>
                              {quotation.supplierName} - ₹
                              {item?.unitPrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </option>
                          ))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => handleRemarksChange(firstItem.id!, e.target.value)}
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Approval Details</h2>
              ...
              {showJustification && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <p className="text-sm font-medium text-yellow-800 mb-3">
                    Please provide justification for not selecting the lowest bid:
                  </p>
                  <textarea
                    rows={3}
