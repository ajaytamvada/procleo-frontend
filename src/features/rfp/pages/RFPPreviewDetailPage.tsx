...
export const RFPPreviewDetailPage: React.FC = () => {
  ...
  // Fetch data
  const { data: rfp, isLoading: isLoadingRFP, error: rfpError } = useRFPForPreview(rfpIdNum);
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-6 print:p-0">
        <div ref={printRef} className="bg-white rounded-lg border border-gray-200 p-8 max-w-5xl mx-auto print:border-0 print:rounded-none print:max-w-none">
          {/* Document Header */}
              ...
              <tr>
                <td className="border border-gray-300 p-4 w-1/3 align-top">
                  ...
                  <div className="text-sm">
                    <p className="font-bold text-base mb-1">{company?.name || 'Company Name'}</p>
                    {company?.cst && <p className="text-xs">GST: {company.cst}</p>}
                  </div>
                </td>
                ...
                <td className="border border-gray-300 p-4 w-1/3 align-top">
                  <div className="text-sm">
                    <p className="font-bold text-base mb-1">{vendor?.name || 'Supplier Name'}</p>
                    {vendor?.address1 && <p className="text-xs">{vendor.address1}</p>}
                    {vendor?.address2 && <p className="text-xs">{vendor.address2}</p>}
                    {vendor?.pinCode && <p className="text-xs">PIN: {vendor.pinCode}</p>}
                  </div>
                </td>
              </tr>
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td className="pt-3" colSpan={2}>
                          <strong>NOTE:</strong> Please submit your most competitive offer for the supply of the following items furnishing all the required details in the format as under
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-gray-50">
                        ...
                        <th className="border border-gray-300 px-4 py-2 text-left font-semibold w-32">Quantity</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rfp.items && rfp.items.length > 0 ? (
                        rfp.items.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td className="border border-gray-300 px-4 py-2">{item.quantity} {item.unitOfMeasurement || ''}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td colSpan={2} className="border border-gray-300 px-4 py-4 text-center text-gray-500">
                            No items found
                          </td>
                        </tr>
                      )}
              ...
              <tr>
                <td colSpan={3} className="border border-gray-300 p-4 text-sm">
                  <div className="space-y-2">
                    <p><strong>Closing Date:</strong> {new Date(rfp.closingDate).toLocaleDateString()}</p>
