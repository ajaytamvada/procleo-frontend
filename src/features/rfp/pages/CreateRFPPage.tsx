...
import { ArrowLeft, Plus, Trash2, Save, Send, Calendar, ChevronDown } from 'lucide-react';
...
const CreateRFPPage: React.FC = () => {
  ...
  const handleItemChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    const numValue = name === 'quantity' || name === 'indicativePrice' || name === 'unitPrice' 
      ? parseFloat(value) || 0 
      : value;
  ...
  const addItem = () => {
    if (newItem.itemName && newItem.quantity > 0) {
      setFormData(prev => ({
        ...prev,
        items: [...prev.items, { ...newItem, grandTotal: newItem.quantity * (newItem.unitPrice || 0) }]
      }));
      setNewItem({
        itemName: '',
        itemCode: '',
  ...
  const calculateTotalAmount = () => {
    return formData.items.reduce((sum, item) => sum + (item.grandTotal || 0), 0);
  };
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Indicative Price</label>
                    <input
                      type="number"
                    ...
                    {formData.items.length === 0 ? (
                      ...
                      formData.items.map((item, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-500">{item.prNumber || formData.prNumber || '-'}</td>
