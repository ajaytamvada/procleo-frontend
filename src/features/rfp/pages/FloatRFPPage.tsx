...
export const FloatRFPPage: React.FC = () => {
  ...
  const handleFloatRFP = () => {
    ...
    floatRFPMutation.mutate(
      {
        rfpId,
        supplierIds: selectedSuppliers.size > 0 ? Array.from(selectedSuppliers) : undefined,
        unregisteredVendors: unregisteredEmails.length > 0 ? unregisteredEmails : undefined,
      },
      {
        onSuccess: () => {
          navigate('/rfp/manage');
  ...
  const handleAddEmail = () => {
    ...
    if (unregisteredEmails.some(e => e.email.toLowerCase() === newEmail.trim().toLowerCase())) {
      ...
      <div className='bg-white border-b border-gray-200 px-6 py-4'>
        <div className='flex items-center justify-between'>
          ...
          <button
            ...
            disabled={
              (selectedSuppliers.size === 0 && unregisteredEmails.length === 0) || floatRFPMutation.isPending
            }
          ...
          >
            {floatRFPMutation.isPending ? (
              ...
              <>
                <Send size={16} />
                <span>Float RFP ({selectedSuppliers.size + unregisteredEmails.length})</span>
              </>
            )}
          </button>
        </div>
        ...
        <div className='flex flex-wrap gap-3 items-start'>
          <div className='flex-1 min-w-0'>
            <input
              ...
              className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 ${emailError ? 'border-red-500' : 'border-gray-300'
                }`}
            />
            {emailError && (
                  ...
                  <div className='divide-y divide-gray-200'>
                    {availableVendors.map(vendor => (
                      <div
                        ...
                        className={`p-3 cursor-pointer transition-colors ${availableSelected.has(vendor.id)
                  ...
                  <div className='divide-y divide-gray-200'>
                    {selectedVendors.map(vendor => (
                      <div
                        ...
                        className={`p-3 cursor-pointer transition-colors ${selectedListSelected.has(vendor.id)
                          ? 'bg-purple-50 border-l-4 border-purple-500'
                          : 'hover:bg-gray-100'
                          }`}
                      >
