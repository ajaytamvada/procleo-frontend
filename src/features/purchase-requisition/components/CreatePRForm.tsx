...
export const CreatePRForm: React.FC<CreatePRFormProps> = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      requestedDate: initialData?.requestedDate || new Date().toISOString().split('T')[0],
  ...
  const handleItemUpdate = (index: number, item: Omit<PurchaseRequisitionItem, 'id' | 'subTotal'>) => {
    update(index, item);
  };
  ...
  const onSubmitForm: SubmitHandler<CreatePRFormData> = (data: CreatePRFormData) => {
    onSubmit(data, false);
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f8f9fa' }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold" style={{ color: '#1a0b2e' }}>Create Purchase Requisition</h1>
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)'; }}
      ...
      <form id="create-pr-form" onSubmit={handleSubmit(onSubmitForm)} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {/* Basic Information */}
        <div style={{
          backgroundColor: 'white',
          borderRadius: '12px',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              {errors.requestor && (
                <p className="mt-1 text-sm text-red-600">{errors.requestor.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.approvalGroup && (
                <p className="mt-1 text-sm text-red-600">{errors.approvalGroup.message}</p>
              )}
            </div>
          </div>
        ...
        <div style={{
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold" style={{ color: '#1a0b2e' }}>Item Details</h3>
            <button
              type="button"
          ...
          <PRItemsTable
            items={fields.map((field, index) => ({
              ...field,
              id: field.id,
              subTotal: calculateSubTotal(watchedItems[index]?.quantity || 0, watchedItems[index]?.indicativePrice || 0),
            }))}
          ...
          <div className="mt-6 pt-4" style={{ borderTop: '1px solid #e5e7eb' }}>
            <div className="flex justify-end">
              <div className="text-xl font-semibold" style={{ color: '#1a0b2e' }}>
