...
export const PRStatusPage: React.FC = () => {
  ...
  const { data: statusItems = [], isLoading, isError } = usePRStatus(debouncedSearch || undefined);
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedItems.map((item, index) => (
                <tr key={`${item.requestNumber}-${item.itemId}-${index}`} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 text-sm text-gray-900 max-w-xs truncate" title={item.model}>
                    {item.model || '-'}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                    {item.approvedBy || 'NA'}
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <Badge className={getStatusColor(item.rmApprovalStatus || item.approvalStatus)}>
                      {(item.rmApprovalStatus || item.approvalStatus || 'Pending').charAt(0).toUpperCase() +
                       (item.rmApprovalStatus || item.approvalStatus || 'Pending').slice(1)}
                    </Badge>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                    {item.remarks || '-'}
