...
>(({ 
  ...
  const [switchId] = React.useState(() => id || `switch-${Math.random().toString(36).substr(2, 9)}`);
  ...
  const switchElement = (
    <SwitchPrimitive.Root
      ...
      className={cn(
        ...
        hasError && 'data-[state=unchecked]:bg-destructive/20 focus-visible:ring-destructive',
        className
      )}
      ...
      <div className="space-y-2">
        <div className="flex items-start justify-between space-x-3">
          <div className="grid gap-1.5 leading-none">
            ...
            {description && (
              <p className="text-sm text-muted-foreground" id={`${switchId}-description`}>
                {description}
              </p>
            )}
          </div>
