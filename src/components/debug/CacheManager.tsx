...
export function CacheManagerDebug() {
  ...
  const { clearCache, performCleanup, warmCache, getDatabaseSize, exportData } = useCacheManagement();
  const { isSyncing, lastSync, syncData, getStaleData, getPendingMutations } = useDataSync();
    ...
    <div className="space-y-6 p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {metrics.lastCleanup ? new Date(metrics.lastCleanup).toLocaleTimeString() : 'Never'}
              </div>
          ...
          <div className="space-y-3">
            {dbSize.map((store) => (
              <div key={store.store} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <div className="font-medium capitalize">{store.store.replace(/([A-Z])/g, ' $1')}</div>
                  <div className="text-sm text-gray-600">{store.count} items</div>
                </div>
                <div className="text-right">
                  <div className="font-medium">{formatBytes(store.size)}</div>
                  <div className="text-sm text-gray-600">
                    {totalDbSize > 0 ? formatPercentage(store.size / totalDbSize) : '0%'}
                  </div>
                </div>
              </div>
            ))}
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-gray-600">
            {pendingMutations.length} pending mutations, {staleData.length} stale entities
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Pending Mutations */}
          <div>
            <h4 className="font-medium mb-2">Queued Mutations ({queuedMutations.length})</h4>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {queuedMutations.slice(0, 5).map((mutation) => (
                <div key={mutation.id} className="p-2 bg-yellow-50 rounded border-l-4 border-yellow-400">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-medium text-sm">{mutation.description}</div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Button 
              onClick={() => warmCache(['dashboard', 'purchase-orders', 'user'])} 
              size="sm" 
              variant="outline"
            >
              Warm Cache
